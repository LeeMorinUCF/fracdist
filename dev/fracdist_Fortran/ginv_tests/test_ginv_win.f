...
c Set parameters for number of test cases.
      ...
      iscon = 0
c
c Read list of probabilities from any one of the tables.
c
...
c
      do iq=1,nq
c       Calculate ginv for all lines together (for this rank iq).
        call gcinv(iq,np,probs,ginv)
c       Record this column in the matrix ginviq.
        do ip=1,np
          ginviq(ip,iq)=ginv(ip)
        end do
      end do
c
